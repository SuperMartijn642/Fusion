...
public interface QuadProcessor<S> {
     ...
     * @param properties     property store containing model properties as well as shared properties. May be used to store data shared between multiple quads. Any stored properties are released after rendering.
     */
    S extractState(@Nullable IBlockAccess level, @Nullable BlockPos pos, @Nullable IBlockState state, Supplier<Random> randomSupplier, PropertyStore properties);
     ...
     * @param state      state obtained from {@link #extractState(IBlockAccess, BlockPos, IBlockState, Supplier, PropertyStore)}
     * @param properties property store containing model properties as well as shared properties. May be used to store data shared between multiple quads. Any stored properties are released after rendering.
     */
    @Nullable
