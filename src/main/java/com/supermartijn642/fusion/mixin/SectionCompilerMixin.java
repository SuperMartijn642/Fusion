...
public class SectionCompilerMixin {
    ...
    @Shadow
    private BufferBuilder getOrBeginLayer(Map<RenderType,BufferBuilder> bufferStore, SectionBufferBuilderPack bufferCreator, RenderType renderType){
    ...
    private BlockStateModel collectModelsByOffset(BlockStateModel model,
                                                  ...
                                                  @Local BlockState blockState, @Local(ordinal = 2) BlockPos pos, @Local RenderChunkRegion renderRegion, @Local PoseStack poseStack, @Local List<BlockModelPart> parts, @Local RandomSource randomSource, @Local(ordinal = 0) Map<BlockPos,ModelData> modelDataMap, @Local(ordinal = 1) Map<RenderType,BufferBuilder> bufferStore, @Local SectionBufferBuilderPack bufferCreator){
        ...
        try{
            ...
            modelsByRandomOffset.foreach(
                entry -> {
                    ...
                    for(RenderType renderType : entry.getRenderTypes(blockState, randomSource, modelData)){
