...
public class FusionEntityModelLoader {
    ...
    public static ResourceLocation locationForLayer(ModelLayerLocation layer){
        return layer.getModel().withSuffix("/" + layer.getLayer().replace(':', '/'));
    }
    ...
    private static ModelPart loadModel(ResourceLocation location, Resource resource, EntityModelLoader loader){
        ...
        try(Reader input = resource.openAsReader()){
            ...
            throw new RuntimeException("Failed to read resource for '" + location + "' from pack '" + resource.source().packId(), e);
        }catch(JsonSyntaxException e){
            FusionClient.LOGGER.error("Failed to parse json for '{}' from pack '{}': {}", location, resource.source().packId(), e.getMessage());
        }
        if(json == null)
            return null;
        ...
        try{
            ...
            FusionClient.LOGGER.error("Failed to load entity model for '{}' from pack '{}': {}", location, resource.source().packId(), e.getMessage());
        }
        return null;
    }
}
