 ...
 */
public class CuboidModelSerializer implements JsonSerializer<CuboidModel> {
    ...
    public JsonElement serialize(CuboidModel src, Type typeOfSrc, JsonSerializationContext context){
        ...
        if(!src.textureSlots().values().isEmpty()){
            ...
            src.textureSlots().values().forEach((key, texture) -> {
                if(texture instanceof TextureSlots.Value && ((TextureSlots.Value)texture).material().forceTranslucent()){
                    Material material = ((TextureSlots.Value)texture).material();
                    JsonObject materialData = new JsonObject();
                    materialData.addProperty("sprite", material.sprite().toString());
                    materialData.addProperty("force_translucent", true);
                    textures.add(key, materialData);
                }else{
                    String value = texture instanceof TextureSlots.Value ?
                        ((TextureSlots.Value)texture).material().sprite().toString() :
