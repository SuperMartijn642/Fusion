...
public class CuboidGeometryElementImpl implements CuboidModelGeometry.Element {
    ...
    public static CuboidModelGeometry.Element of(BlockElement element){
        return builder()
            ...
            .ambientOcclusion(element.getFaceData().ambientOcclusion() ? null : false)
            .property(ModelProperty.NEO_GEOMETRY_COLOR, element.getFaceData().color())
            .property(ModelProperty.NEO_GEOMETRY_BLOCK_LIGHT, element.getFaceData().blockLight())
            .property(ModelProperty.NEO_GEOMETRY_SKY_LIGHT, element.getFaceData().skyLight())
            .property(ModelProperty.NEO_GEOMETRY_AMBIENT_OCCLUSION, element.getFaceData().ambientOcclusion())
            .build();
    }
    ...
    private CuboidGeometryElementImpl(Vector3fc from, Vector3fc to, BlockElementRotation rotation, Map<Direction,CuboidModelGeometry.Face> faces, Boolean shade, Integer lightEmission, Boolean ambientOcclusion, Boolean emissive, Map<ModelProperty<?,?>,Function<?,?>> properties){
        this.from = from;
        this.to = to;
