...
public class RandomTextureType implements TextureType<RandomTextureData,StitchedRandomTextureData> {
    ...
    public @Nullable QuadProcessor<?> initializeModelQuad(MutableQuad quad, SpriteInstance sprite, StitchedRandomTextureData data, PropertyStore properties){
        ...
        return new QuadProcessor<Pair<Integer,Object>>() {
            ...
            @Override
            public Pair<Integer,Object> extractState(@Nullable IBlockAccess level, @Nullable BlockPos pos, @Nullable IBlockState state, Supplier<Random> randomSupplier, PropertyStore properties){
    ...
    private static int calculateTileIndex(int tileCount, EnumFacing side, @Nullable BlockPos pos, Random random, RandomTextureData data){
        if(pos == null)
            pos = BlockPos.ORIGIN;
        // Calculate seed
        long seed = 1;
