...
public abstract class FusionItemModelModifierProvider implements DataProvider {
    ...
    public FusionItemModelModifierProvider(String modid, PackOutput output){
        this.modName = net.neoforged.fml.ModList.get().getModContainerById(modid).map(net.neoforged.fml.ModContainer::getModInfo).map(net.neoforged.neoforgespi.language.IModInfo::getDisplayName).orElse(modid);
        this.output = output;
    }
