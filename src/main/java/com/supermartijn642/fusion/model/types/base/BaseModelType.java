...
public abstract class BaseModelType<T extends BaseModelData, BUILDER extends BaseModelData.Builder<BUILDER,T>> implements ModelType<T> {
    ...
    public BlockStateModel bakeBlockStateModel(BlockStateModelBakingContext context, T data){
        ...
        if(particleMaterial == null){
            ...
            particleMaterial = ModelMaterial.missingBlockOrItemAtlas();
        }
    ...
    public ItemModel bakeItemModel(ItemModelBakingContext context, T data){
        ...
        return new BaseItemModel(parts, context.getTintSources());
    }
        ...
        for(String key : json.keySet()){
            if(json.get(key).isJsonPrimitive() && json.getAsJsonPrimitive(key).isString()){
                ...
                if(!value.isEmpty() && value.charAt(0) == '#')
                    ...
                    materials.put(key, Either.right(ModelMaterial.blockOrItemAtlas(Identifier.parse(value))));
                else
    ...
    protected BlockElementRotation deserializeRotation(JsonObject json){
        ...
        if(json.has("axis") || json.has("angle")){
            ...
            rotationValue = new BlockElementRotation.SingleAxisRotation(axis, json.get("angle").getAsFloat());
    ...
    protected JsonObject serializeRotation(BlockElementRotation rotation){
        ...
        }else if(rotation.value() instanceof BlockElementRotation.EulerXYZRotation(float x, float y, float z)){
            if(x != 0)
    ...
    protected JsonObject serializeFace(CuboidModelGeometry.Face face){
        ...
        BlockElementFace.UVs uv = face.uv();
        if(uv != null){
