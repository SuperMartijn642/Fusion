...
public class ModelStackImpl implements ModelStack {
    ...
    public @NotNull Iterator<UntypedModelInstance> iterator(){
        return new Iterator<UntypedModelInstance>() {
            int index = 0;
    ...
    @Override
    public @Nullable Boolean findIsGui3d(){
        for(ModelStackImpl entry : this.stack){
            Boolean value = entry.model.getIsGui3d();
            if(value != null)
                return value;
        }
        return null;
    ...
    @Override
    public @Nullable Boolean findIsGui3dIncludingParents(ModelResolver modelResolver){
        return this.walkStackAndParents(
            modelResolver,
            (modelInstance, stack) -> {
    ...
    @Override
    public @Nullable ItemTransformVec3f findItemTransform(ItemCameraTransforms.TransformType type){
        for(ModelStackImpl entry : this.stack){
            ItemTransformVec3f value = entry.model.getItemTransform(type);
            if(value != null)
                return value;
        }
        return null;
    ...
    @Override
    public @Nullable ItemTransformVec3f findItemTransformIncludingParents(ItemCameraTransforms.TransformType type, ModelResolver modelResolver){
        return this.walkStackAndParents(
            modelResolver,
            (modelInstance, stack) -> {
                ItemTransformVec3f v = modelInstance.getItemTransform(type);
                return v == null ? ModelWalker.Result.proceed() : ModelWalker.Result.stop(v);
            }
        ).orElse(null);
    }
    ...
    public @Nullable Either<String,ModelMaterial> findMaterialIncludingParents(String key, ModelResolver modelResolver){
        return this.walkStackAndParents(
            modelResolver,
            (modelInstance, stack) -> {
                Either<String,ModelMaterial> v = modelInstance.getMaterial(key);
                return v == null ? ModelWalker.Result.proceed() : ModelWalker.Result.stop(v);
            }
        ).orElse(null);
    }
    ...
    public @Nullable ModelGeometry findGeometryIncludingParents(ModelResolver modelResolver){
        return this.walkStackAndParents(
            modelResolver,
            (modelInstance, stack) -> {
                ModelGeometry v = modelInstance.getGeometry();
                return v == null ? ModelWalker.Result.proceed() : ModelWalker.Result.stop(v);
            }
        ).orElse(null);
    }
    ...
    public @Nullable Boolean findShadeIncludingParents(ModelResolver modelResolver){
        return this.walkStackAndParents(
            modelResolver,
            (modelInstance, stack) -> {
                Boolean v = modelInstance.getShade();
                return v == null ? ModelWalker.Result.proceed() : ModelWalker.Result.stop(v);
            }
        ).orElse(null);
    }
    ...
    public @Nullable Boolean findEmissiveIncludingParents(ModelResolver modelResolver){
        return this.walkStackAndParents(
            modelResolver,
            (modelInstance, stack) -> {
                Boolean v = modelInstance.getEmissive();
                return v == null ? ModelWalker.Result.proceed() : ModelWalker.Result.stop(v);
            }
        ).orElse(null);
    }
    ...
    public <T> Optional<T> walkStack(ModelWalker<T> walker){
        ...
        return result instanceof ModelWalkerImpl.StopResult<?> ? Optional.ofNullable(((ModelWalkerImpl.StopResult<T>)result).value()) : Optional.empty();
    }
    ...
    public <T> Optional<T> walkStackAndParents(ModelResolver modelResolver, ModelWalker<T> walker){
        ...
        if(result instanceof ModelWalkerImpl.StopResult)
            return Optional.ofNullable(((ModelWalkerImpl.StopResult<T>)result).value());
