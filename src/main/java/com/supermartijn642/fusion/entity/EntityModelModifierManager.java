        ...
        if(properties != null){
            for(ModelLayerLocation location : properties.getLayers().keySet()){
                if(!overwrittenModelParts.containsKey(location))
                    FusionClient.LOGGER.warn("An entity model modifier for entity '{}' overwrites layer '{}', but no such layer was found!", Registry.ENTITY_TYPE.getKey(entityType), location);
            }
        }
            ...
            for(Map.Entry<ModelLayerLocation,FusionModelPart> entry : overwrittenModelParts.entrySet()){
                ...
                for(ModelPart model : models){
                    ...
                    if(!missingParts.isEmpty()){
                        FusionClient.LOGGER.error("A model for layer '{}' on entity '{}' is missing required parts: {}!", layer, Registry.ENTITY_TYPE.getKey(entityType), missingParts.stream().map(s -> '\'' + s + '\'').collect(Collectors.joining(",")));
                        missingParts.clear();
