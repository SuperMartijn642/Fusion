...
public class IrisPBRTextureCreationHandler {
    ...
    private static final Set<Identifier> PROCESSED_TEXTURES = new HashSet<>();
    ...
    private static final Field pbrSpriteContentsField;
    ...
    static{
        try{
            ...
            Constructor<?> constructor = pbrSpriteContents.getDeclaredConstructor(Identifier.class, FrameSize.class, NativeImage.class, ResourceMetadata.class, PBRType.class);
            constructor.setAccessible(true);
            pbrSpriteContentsConstructor = constructor;

            // PBRTextureAtlasSprite
            constructor = AtlasPBRLoader.PBRTextureAtlasSprite.class.getDeclaredConstructor(Identifier.class, pbrSpriteContents, int.class, int.class, int.class, int.class, TextureAtlasSprite.class);
            constructor.setAccessible(true);
            pbrTextureAtlasSpriteConstructor = constructor;

            pbrSpriteContentsField = AtlasPBRLoader.PBRTextureAtlasSprite.class.getDeclaredField("pbrContents");
            pbrSpriteContentsField.setAccessible(true);
        }catch(Exception e){
     ...
     */
    private static AtlasPBRLoader.PBRTextureAtlasSprite instantiatePBRTextureAtlasSprite(Identifier name, FrameSize size, NativeImage image, ResourceMetadata metadata, PBRType pbrType, int atlasWidth, int atlasHeight, int x, int y, TextureAtlasSprite baseSprite, FusionSpriteContents fusionContents){
        try{
            Object spriteContents = pbrSpriteContentsConstructor.newInstance(name, size, image, metadata, pbrType);
            AtlasPBRLoader.PBRTextureAtlasSprite sprite = (AtlasPBRLoader.PBRTextureAtlasSprite)pbrTextureAtlasSpriteConstructor.newInstance(name, spriteContents, atlasWidth, atlasHeight, x, y, baseSprite);
            sprite.contents = fusionContents;
            pbrSpriteContentsField.set(sprite, fusionContents);
            return sprite;
    ...
    public static void createPBRTexture(TextureInstance<?> textureInstance, ResourceManager resourceManager, int atlasWidth, int atlasHeight, int mipLevel, PBRType pbrType, Supplier<PBRAtlasTexture> pbrAtlas){
        ...
        // Read pbr image
        Identifier identifier = textureInstance.getIdentifier();
        Identifier pbrIdentifier = identifier.withPath(pbrType.appendSuffix(identifier.getPath()));
        Identifier pbrImageLocation = pbrIdentifier.withPrefix("textures/").withSuffix(".png");
        ...
        Identifier regularImageLocation = identifier.withPrefix("textures/").withSuffix(".png");
        ...
        AnimationMetadataSection animationMetadata = metadata.getSection(AnimationMetadataSection.TYPE).orElse(null);
        // Get texture metadata
        TextureMetadataSection textureMetadata = metadata.getSection(TextureMetadataSection.TYPE).orElse(null);

        // Get the fusion metadata
        //noinspection unchecked,rawtypes
        RawTextureInstance<Object,Object> rawTexture = (RawTextureInstance)metadata.getSection(FusionTextureMetadataSection.TYPE).orElse(null);
        if(rawTexture == null){
        ...
        try{
            createPBRSprites(output, textureMetadata, textureInstance, atlasWidth, atlasHeight, mipLevel, pbrType, sprites::add);
        }catch(UserErrorException e){
    ...
    private static void createPBRSprites(TextureOutputImpl<?> textureOutput, TextureMetadataSection textureMetadata, TextureInstance<?> regularTexture, int atlasWidth, int atlasHeight, int mipLevel, PBRType pbrType, Consumer<AtlasPBRLoader.PBRTextureAtlasSprite> spriteConsumer) throws UserErrorException{
        ...
        for(int i = 0; i < textureOutput.getSubTextures().size(); i++){
            ...
            try{
                createPBRSprites(subTextureOutput, textureMetadata, regularSubTexture, atlasWidth, atlasHeight, mipLevel, pbrType, spriteConsumer);
            }catch(UserErrorException e){
        ...
        for(SpriteBuilderImpl spriteBuilder : textureOutput.getSprites()){
            ...
            Identifier identifier = spriteBuilder.getIdentifier();
            ...
            FusionSpriteContents contents = new FusionSpriteContents(identifier, (SpriteImageSourceImpl)spriteBuilder.getImageSource(), List.of(), Optional.ofNullable(textureMetadata));
