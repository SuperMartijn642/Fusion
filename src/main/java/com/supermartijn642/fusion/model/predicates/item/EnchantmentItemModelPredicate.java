...
public class EnchantmentItemModelPredicate implements ItemModelPredicate {
    ...
    public static final Serializer<EnchantmentItemModelPredicate> SERIALIZER = new Serializer<>() {
        ...
        public EnchantmentItemModelPredicate deserialize(JsonObject json) throws JsonParseException{
            ...
            if(!BuiltInRegistries.ENCHANTMENT.containsKey(enchantment))
    ...
    private EnchantmentItemModelPredicate(ResourceLocation enchantment, int minLevel, int maxLevel){
    ...
    public boolean test(ItemStack stack){
        ...
        ListTag enchantmentsTag = stack.is(Items.ENCHANTED_BOOK) ? EnchantedBookItem.getEnchantments(stack) : stack.getEnchantmentTags();
        if(!enchantmentsTag.isEmpty()){
            for(int i = 0; i < enchantmentsTag.size(); i++){
                CompoundTag tag = enchantmentsTag.getCompound(i);
                int level = EnchantmentHelper.getEnchantmentLevel(tag);
                if(level >= this.minLevel && level <= this.maxLevel && this.enchantment.equals(EnchantmentHelper.getEnchantmentId(tag)))
                    return true;
            }
        }
        return this.minLevel == 0;
