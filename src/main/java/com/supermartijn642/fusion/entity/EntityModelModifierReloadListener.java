...
public class EntityModelModifierReloadListener {
    ...
    public static void reload(ResourceManager resourceManager){
        ...
        for(Map.Entry<ResourceLocation,Resource> entry : resourceManager.listResources(LOCATION, l -> l.getPath().endsWith(".json")).entrySet()){
            ...
            try(Reader reader = entry.getValue().openAsReader()){
                ...
                FusionClient.LOGGER.error("Failed to parse json from entity model modifier '{}'!", entry.getKey(), e);
            }catch(Exception e){
                FusionClient.LOGGER.error("Encountered an exception whilst reading entity model modifier at '{}'!", entry.getKey(), e);
            }
        }
    ...
    private static void parseResource(JsonObject json){
        ...
        for(EntityType<?> target : targets){
            ...
            ResourceLocation entityIdentifier = Registry.ENTITY_TYPE.getKey(target);
    ...
    private static ModelOption parseModelOption(JsonElement element, String propertyName){
        if(element.isJsonPrimitive() && element.getAsJsonPrimitive().isString()){
            ...
            return new ModelOption(Either.left(new ResourceLocation(element.getAsString())), null, null, null, null, null, null, null, null);
        }
        if(element.isJsonObject()){
