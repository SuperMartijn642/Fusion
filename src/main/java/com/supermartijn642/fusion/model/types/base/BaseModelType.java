...
public abstract class BaseModelType<T extends BaseModelData, BUILDER extends BaseModelData.Builder<BUILDER,T>> implements ModelType<T> {
    ...
    public static void applyTextureProperties(MutableQuad quad){
        TextureInstance<?> textureInstance = SpriteHelper.getTextureInstance(quad.sprite());
        if(textureInstance != null && textureInstance.getCustomData() instanceof BaseTextureData data){
            if(data.isEmissive())
                quad.emissive(true);
            FusionClient.getChunkLayer(data.getRenderType()).ifPresent(quad::chunkRenderType);
    ...
    protected void deserialize(JsonObject json, BUILDER builder){
        ...
        if(json.has("gui_light")){
            ...
            try{
                ...
                String allowedValues = Arrays.stream(BlockModel.GuiLight.values()).map(o -> "'" + o.name().toLowerCase(Locale.ROOT) + "'").collect(Collectors.joining(","));
    ...
    protected ItemTransforms deserializeItemTransforms(JsonObject json){
        ItemTransform thirdPersonRightHand = this.deserializeItemTransform(json, "thirdperson_righthand", ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND);
        ItemTransform thirdPersonLeftHand = this.deserializeItemTransform(json, "thirdperson_lefthand", ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND);
        if(thirdPersonLeftHand == ItemTransform.NO_TRANSFORM)
            thirdPersonLeftHand = thirdPersonRightHand;
        ItemTransform firstPersonRightHand = this.deserializeItemTransform(json, "firstperson_righthand", ItemTransforms.TransformType.FIRST_PERSON_RIGHT_HAND);
        ItemTransform firstPersonLeftHand = this.deserializeItemTransform(json, "firstperson_lefthand", ItemTransforms.TransformType.FIRST_PERSON_LEFT_HAND);
        if(firstPersonLeftHand == ItemTransform.NO_TRANSFORM)
            firstPersonLeftHand = firstPersonRightHand;
        ItemTransform head = this.deserializeItemTransform(json, "head", ItemTransforms.TransformType.HEAD);
        ItemTransform gui = this.deserializeItemTransform(json, "gui", ItemTransforms.TransformType.GUI);
        ItemTransform ground = this.deserializeItemTransform(json, "ground", ItemTransforms.TransformType.GROUND);
        ItemTransform fixed = this.deserializeItemTransform(json, "fixed", ItemTransforms.TransformType.FIXED);
    ...
    protected JsonObject serializeItemTransforms(BaseModelData data){
        JsonObject json = new JsonObject();
        this.serializeItemTransform(json, "thirdperson_lefthand", data.getItemTransform(ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND));
        this.serializeItemTransform(json, "thirdperson_righthand", data.getItemTransform(ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND));
        this.serializeItemTransform(json, "firstperson_lefthand", data.getItemTransform(ItemTransforms.TransformType.FIRST_PERSON_LEFT_HAND));
        this.serializeItemTransform(json, "firstperson_righthand", data.getItemTransform(ItemTransforms.TransformType.FIRST_PERSON_RIGHT_HAND));
        this.serializeItemTransform(json, "head", data.getItemTransform(ItemTransforms.TransformType.HEAD));
        this.serializeItemTransform(json, "gui", data.getItemTransform(ItemTransforms.TransformType.GUI));
        this.serializeItemTransform(json, "ground", data.getItemTransform(ItemTransforms.TransformType.GROUND));
        this.serializeItemTransform(json, "fixed", data.getItemTransform(ItemTransforms.TransformType.FIXED));
        return json;
    }

    protected ItemTransform deserializeItemTransform(JsonObject json, String name, ItemTransforms.TransformType type){
        if(!json.has(name))
