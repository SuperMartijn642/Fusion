...
public interface CullableQuads {
    ...
    /**
     * Converts the given {@link QuadCollection} to a {@link CullableQuads}.
     */
    static CullableQuads of(QuadCollection quads){
        return builder()
            .add(null, quads.getQuads(null).stream().<QuadAccess>map(MutableQuad::create).toList())
            .add(Direction.UP, quads.getQuads(Direction.UP).stream().<QuadAccess>map(MutableQuad::create).toList())
            .add(Direction.DOWN, quads.getQuads(Direction.DOWN).stream().<QuadAccess>map(MutableQuad::create).toList())
            .add(Direction.NORTH, quads.getQuads(Direction.NORTH).stream().<QuadAccess>map(MutableQuad::create).toList())
            .add(Direction.EAST, quads.getQuads(Direction.EAST).stream().<QuadAccess>map(MutableQuad::create).toList())
            .add(Direction.SOUTH, quads.getQuads(Direction.SOUTH).stream().<QuadAccess>map(MutableQuad::create).toList())
            .add(Direction.WEST, quads.getQuads(Direction.WEST).stream().<QuadAccess>map(MutableQuad::create).toList())
            .build();
    }
    ...
    /**
     * Converts this collection to a vanilla quad collection.
     */
    QuadCollection toQuadCollection();
