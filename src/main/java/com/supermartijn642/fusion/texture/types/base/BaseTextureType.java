...
public class BaseTextureType implements TextureType<BaseTextureData,BaseTextureData> {
    ...
    public static Set<BlockRenderLayer> getChunkLayers(BaseTextureData data){
        return data.getRenderType() == null ? Collections.emptySet() : ImmutableSet.of(getChunkLayer(data.getRenderType()));
    }
    ...
    public static void applyProperties(MutableQuad quad, BaseTextureData data){
        ...
        if(data.getRenderType() != null)
            quad.renderLayer(getChunkLayer(data.getRenderType()));
        if(data.getTinting() != null)
            quad.tintIndex(39216);
    }

    public static BlockRenderLayer getChunkLayer(BaseTextureData.RenderType renderType){
        if(renderType == null)
            return null;
        switch(renderType){
            case OPAQUE:
