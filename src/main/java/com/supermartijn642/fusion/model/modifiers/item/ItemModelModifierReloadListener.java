...
public class ItemModelModifierReloadListener {
    ...
    private final Map<ResourceLocation,List<Properties>> modifiers = new HashMap<>();
    ...
    public void applyModelModifiers(ModelBakery.BakingResult results, ItemModel.BakingContext bakingContext){
        ...
        Function<ResourceLocation,ItemModel> modelResolver = new Function<>() {
            final Map<ResourceLocation,ItemModel> models = new HashMap<>();

            @Override
            public ItemModel apply(ResourceLocation identifier){
                ItemModel model = this.models.get(identifier);
                if(model != null)
                    return model;
                try{
                    model = new BlockModelWrapper.Unbaked(identifier, List.of()).bake(bakingContext);
                }catch(Exception e){
                    FusionClient.LOGGER.error("Caught exception baking model '{}':", identifier, e);
                    model = results.missingItemModel();
                }
        ...
        for(Map.Entry<ResourceLocation,List<Properties>> entry : this.modifiers.entrySet()){
    ...
    private static List<ItemModelModifierItemModel.ConditionalModel> createConditionModels(List<ModelEntry> modelEntries, Function<ResourceLocation,ItemModel> modelResolver){
    ...
    private void parseResource(JsonObject json, ResourceLocation location){
        ...
        if(json.has("default_model")){
            ...
            defaultModel = List.of(ModelEntry.simple(ResourceLocation.parse(json.get("default_model").getAsString())));
        }
