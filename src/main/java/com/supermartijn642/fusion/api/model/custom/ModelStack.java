...
public interface ModelStack extends Iterable<UntypedModelInstance> {
    ...
    /**
     * Finds the first model in the stack that returns a non-null value for {@link ModelInstance#getItemTransform(ItemTransforms.TransformType)} and returns its value.
     * If no such model is present, the result is {@code null}.
     */
    @Nullable
    ItemTransform findItemTransform(ItemTransforms.TransformType type);

    /**
     * Finds the first model in the stack and the model tree of the last model that returns a non-null value for {@link ModelInstance#getItemTransform(ItemTransforms.TransformType)} and returns its value.
     * If no such model is present, the result is {@code null}.
     */
    @Nullable
    ItemTransform findItemTransformIncludingParents(ItemTransforms.TransformType type, ModelResolver modelResolver);
