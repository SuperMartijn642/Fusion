...
public abstract class AbstractCuboidModelDataBuilder<T extends AbstractCuboidModelDataBuilder<T,S>, S> implements CuboidModelDataBuilder<T,S> {
    ...
    @Override
    public T guiLight(UnbakedModel.GuiLight guiLight){
    ...
    @Override
    public T itemTransform(ItemDisplayContext type, ItemTransform transform){
        if(transform == null)
    ...
    public T itemTransforms(ItemTransforms itemTransforms){
        ...
        for(ItemDisplayContext type : ItemDisplayContext.values()){
            ItemTransform transform = type.isModded() ?
                itemTransforms.moddedTransforms().get(type) :
                itemTransforms.getTransform(type);
            if(transform != null)
                this.itemTransforms.put(type, transform);
        }
        return this.self();
