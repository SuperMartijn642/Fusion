...
public class RandomTextureType implements TextureType<RandomTextureData> {
    ...
    public static void processQuad(MutableQuad quad, BlockPos pos, EnumFacing side, Random random, RandomTextureSprite sprite){
        if(side == null)
            return;
        RandomTextureData data = sprite.data();
        // Determine which tile to use based on position and side
        if(data.getSeed() != null)
            random.setSeed(data.getSeed() ^ ((pos.toLong() + 1) * (side.ordinal() + 1)));
        else
            random.setSeed((pos.toLong() + 1) * (side.ordinal() + 1));
