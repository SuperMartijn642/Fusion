...
public abstract class SimpleModelType<T> implements ModelType<T> {
    ...
    public @Nullable BlockStateModel bakeBlockStateModel(BlockStateModelBakingContext context, ModelStack modelStack, T data){
        ...
        if(geometry != null){
            ...
            ModelGeometry.QuadConsumer quadConsumer = (quad, cullDirection, geometryProperties) -> {
                ...
                if(!forgeRenderType.isEmpty())
                    quad.renderLayers(forgeRenderType.block(), forgeRenderType.entityFabulous());
                // Initialize the quad
        ...
        // Bake parent
        Identifier parent = this.getParent(data);
        if(parent != null){
    ...
    @Nullable
    protected abstract Identifier getParent(T data);
