...
public class CuboidModelGeometryImpl implements CuboidModelGeometry {
    ...
    public static CuboidModelGeometry of(ModelBlock model){
    ...
    public static QuadAccess bakeFace(Face face, Element element, EnumFacing side, ModelTransform transformation, MaterialKeyResolver materialResolver){
    ...
    public static void bakeFace(QuadConsumer consumer, Face face, Element element, EnumFacing side, ModelTransform transformation, MaterialKeyResolver materialResolver){
        ...
        BakedQuad bakedQuad = FACE_BAKERY.makeBakedQuad(
        ...
        EnumFacing cullDirection = face.cullDirection() == null ? null :
            transformation.toTransformation().rotate(face.cullDirection());
        // Emit quad
        consumer.consume(quad, cullDirection, PropertyGetter.compose(face, element));
    }

    private static Face.UV calculateFaceUV(Element element, EnumFacing side){
        ...
        if(side == EnumFacing.DOWN)
            uv = new Face.UV(element.from().x, 16.0F - element.to().z, element.to().x, 16.0F - element.from().z);
        else if(side == EnumFacing.UP)
            uv = new Face.UV(element.from().x, element.from().z, element.to().x, element.to().z);
        else if(side == EnumFacing.NORTH)
            uv = new Face.UV(16.0F - element.to().x, 16.0F - element.to().y, 16.0F - element.from().x, 16.0F - element.from().y);
        else if(side == EnumFacing.SOUTH)
            uv = new Face.UV(element.from().x, 16.0F - element.to().y, element.to().x, 16.0F - element.from().y);
        else if(side == EnumFacing.WEST)
            uv = new Face.UV(element.from().z, 16.0F - element.to().y, element.to().z, 16.0F - element.from().y);
        else if(side == EnumFacing.EAST)
            uv = new Face.UV(16.0F - element.to().z, 16.0F - element.to().y, 16.0F - element.from().z, 16.0F - element.from().y);
