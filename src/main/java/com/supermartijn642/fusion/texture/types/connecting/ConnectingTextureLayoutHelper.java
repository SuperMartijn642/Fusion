...
public class ConnectingTextureLayoutHelper {
    ...
    public static int[] getTilePosition(ConnectingTextureLayout layout, TextureConnections connections){
        return LAYOUT_TO_PROPERTIES[layout.ordinal()].getTile(connections);
    }
    ...
    private enum LayoutProperties {
        ...
        HORIZONTAL(4, 1, 2,
            connections -> (connections.left ? 1 : 0) | (connections.right ? 2 : 0),
            connections -> {
                int[] uv;
        ...
        VERTICAL(1, 4, 2,
            connections -> (connections.top ? 1 : 0) | (connections.bottom ? 2 : 0),
            connections -> {
                int[] uv;
        ...
        final ToIntFunction<TextureConnections> index;
        final int[][] uvs;

        LayoutProperties(int width, int height, int maxIndexSize, ToIntFunction<TextureConnections> index, Function<TextureConnections,int[]> tilePicker){
            this.width = width;
            this.height = height;
            this.index = index;
            ...
            for(TextureConnections connections : TextureConnections.iterateAll()){
                int i = index.applyAsInt(connections);
                if(this.uvs[i] == null)
                    this.uvs[i] = tilePicker.apply(connections);
            }
        }

        int[] getTile(TextureConnections connections){
            return this.uvs[this.index.applyAsInt(connections)];
        }
    }
}
