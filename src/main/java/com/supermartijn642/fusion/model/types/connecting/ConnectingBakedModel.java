...
public class ConnectingBakedModel implements IBakedModel {
    ...
    static{
        ...
        VERTEX_UV_OFFSET = blockFormat.getOffset(blockFormat.getElements().indexOf(DefaultVertexFormats.ELEMENT_UV0)) / 4;
        VERTEX_POSITION_OFFSET = blockFormat.getOffset(blockFormat.getElements().indexOf(DefaultVertexFormats.ELEMENT_POSITION)) / 4;
    ...
    public ConnectingBakedModel(List<ConnectingModelQuad> quads, boolean hasAmbientOcclusion, boolean isGui3d, boolean usesBlockLight, TextureAtlasSprite particleIcon, ItemCameraTransforms transforms, ItemOverrideList overrides){
        ...
        for(ConnectingModelQuad quad : quads){
            ...
            for(int quadIndex = 0; quadIndex < auxiliaryQuadCount + 1; quadIndex++){
                ...
                // Add the block quad
                TaggedBakedQuad finishedQuad = new TaggedBakedQuad(mutableQuad.toBakedQuad(), hasConnectingTexture, quadIndex, predicateIndex, spriteIndex);
        ...
        this.itemModel = new ItemBakedModel(this) {
            ...
            protected List<BakedQuad> getQuads(ItemStack stack, boolean fabulous, @Nonnull Random random, @Nonnull IModelData data, @Nullable RenderType renderType){
                ...
                if(ConnectingBakedModel.this.shouldCheckOriginalItemRenderTypes && RenderTypeLookup.getRenderType(stack, fabulous) == renderType){
    ...
    public IModelData getModelData(@Nonnull IBlockDisplayReader level, @Nonnull BlockPos pos, @Nonnull BlockState state, @Nonnull IModelData data){
        // If there are no predicates, we don't need to collect any world data
    ...
    private static class TaggedBakedQuad {
        ...
        private TaggedBakedQuad(BakedQuad bakedQuad, boolean hasConnectingTexture, int quadIndex, int predicateIndex, int spriteIndex){
