 ...
 */
public class ConnectingBlockStateModel implements BlockStateModel {
    ...
    public ConnectingBlockStateModel(List<Part> parts, TextureAtlasSprite particleSprite){
        this.parts = parts;
    ...
    public void collectParts(@Nullable BlockAndTintGetter level, @Nullable BlockPos pos, @Nullable BlockState state, RandomSource random, List<BlockModelPart> parts){
        ...
        for(Part part : this.parts){
            ...
            Map<RenderType,CullableQuads.Builder> quadsByRenderType = new ChunkRenderTypeMap<>();
            BiConsumer<QuadAccess,Direction> submitter = (quad, cullDirection) -> {
                // Get the quad's render type
                RenderType renderType = quad.chunkRenderType();
                if(renderType == null)
            ...
            for(RenderType renderType : quadsByRenderType.keySet()){
    ...
    public record Part(ConnectingModelQuads quads, TextureAtlasSprite particleSprite) {
    }
}
