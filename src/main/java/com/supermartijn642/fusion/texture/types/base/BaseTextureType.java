...
public class BaseTextureType implements TextureType<BaseTextureData,BaseTextureData> {
    ...
    @Override
    public Set<ChunkSectionLayer> getBlockStateRenderTypes(SpriteInstance sprite, BaseTextureData data){
    ...
    public static Set<ChunkSectionLayer> getChunkLayers(BaseTextureData data){
    ...
    public static void applyProperties(MutableQuad quad, BaseTextureData data){
        ...
        if(data.getRenderType() != null)
            quad.chunkLayer(getChunkLayer(data.getRenderType()));
        if(data.getTinting() != null)
            quad.tintIndex(39216);
    }

    public static ChunkSectionLayer getChunkLayer(BaseTextureData.RenderType renderType){
        if(renderType == null)
            return null;
        return switch(renderType){
            case OPAQUE -> ChunkSectionLayer.SOLID;
