...
public class CompositeModelType implements ModelType<CompositeModelData> {
    ...
    @Override
    public Collection<Identifier> getDependencies(CompositeModelData data){
    ...
    public @Nullable BlockStateModel bakeBlockStateModel(BlockStateModelBakingContext context, ModelStack modelStack, CompositeModelData data){
        ...
        for(List<CompositeModelData.ModelEntry> series : data.getModels()){
            ...
            lists.add(new CompositeBlockStateModel.ConditionalList(List.copyOf(list)));
        }

        // Create model
        if(defaultModel == null)
            defaultModel = context.getMissingBlockStateModel();
    ...
    private static CompositeModelData.ModelEntry deserializeModelEntry(JsonElement element){
        ...
        Either<Identifier,ModelInstance<?>> model;
        if(json.get("model").isJsonPrimitive() && json.getAsJsonPrimitive("model").isString()){
            ...
            model = Either.left(Identifier.parse(json.get("model").getAsString()));
