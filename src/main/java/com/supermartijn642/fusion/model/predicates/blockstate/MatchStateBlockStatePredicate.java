...
public class MatchStateBlockStatePredicate implements BlockStateModelPredicate {
    ...
    public static BlockStateModelPredicate create(IBlockState state, List<BlockPos> offsets){
        ...
        Pair<IProperty<?>,?>[] properties = state.getProperties().entrySet().stream().map(e -> Pair.of(e.getKey(), e.getValue())).toArray(Pair[]::new);
    ...
    public static final Serializer<MatchStateBlockStatePredicate> SERIALIZER = new Serializer<MatchStateBlockStatePredicate>() {
        ...
        public JsonObject serialize(MatchStateBlockStatePredicate value){
            ...
            if(value.blocks.size() == 1)
                json.addProperty("block", ForgeRegistries.BLOCKS.getKey(value.blocks.keySet().iterator().next()).toString());
            else{
