...
public abstract class CombinedBakedModel implements BakedModel {
    ...
    public TriState useAmbientOcclusion(BlockState state, ModelData modelData, RenderType renderType){
        ModelData[] subModelData = modelData.get(SUB_MODEL_DATA);
        return this.getModels().get(0).useAmbientOcclusion(state, subModelData == null ? ModelData.EMPTY : subModelData[0], renderType);
    }
    ...
    public ChunkRenderTypeSet getRenderTypes(BlockState state, RandomSource rand, ModelData modelData){
        ModelData[] subModelData = modelData.get(SUB_MODEL_DATA);
        return this.getModels().get(0).getRenderTypes(state, rand, subModelData == null ? ModelData.EMPTY : subModelData[0]);
    }

    @Override
    public List<RenderType> getRenderTypes(ItemStack stack, boolean fabulous){
        return this.getModels().get(0).getRenderTypes(stack, fabulous);
    }

    @Override
    public List<BakedModel> getRenderPasses(ItemStack stack, boolean fabulous){
        return this.getModels().get(0).getRenderPasses(stack, fabulous);
    }
}
