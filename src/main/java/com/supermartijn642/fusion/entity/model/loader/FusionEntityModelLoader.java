...
public class FusionEntityModelLoader {
        ...
        for(ResourceLocation identifier : identifiers){
            ...
            for(Pair<String,EntityModelLoader> loader : LOADERS){
                ...
                if(optional.isPresent()){
                    ...
                    int index = packOrder.get(r.sourcePackId());
                    if(index > packIndex){
                        resource = r;
    ...
    private static ModelPart loadModel(ResourceLocation location, Resource resource, EntityModelLoader loader){
        ...
        try(Reader input = resource.openAsReader()){
            ...
            throw new RuntimeException("Failed to read resource for '" + location + "' from pack '" + resource.sourcePackId(), e);
        }catch(JsonSyntaxException e){
            FusionClient.LOGGER.error("Failed to parse json for '{}' from pack '{}': {}", location, resource.sourcePackId(), e.getMessage());
        }
        if(json == null)
            return null;
        ...
        try{
            ...
            FusionClient.LOGGER.error("Failed to load entity model for '{}' from pack '{}': {}", location, resource.sourcePackId(), e.getMessage());
        }
        return null;
    }
}
