...
public class FusionBlockModelData extends BlockModel {
    ...
    public static Collection<ResourceLocation> gatherBlockModelMaterials(BlockModel model,
        ...
        ModelGeometry.MaterialResolver.fromKeyLookup(
            ...
            material -> {
                materials.add(material.texture());
                return null;
            },
            key -> missingMaterials.add("#" + key + " in " + topModel.name),
    ...
    private static Map<ResourceLocation,IUnbakedModel> resolveAndGatherMaterials(IUnbakedModel model,
                                                                                 Function<ResourceLocation,IUnbakedModel> modelGetter,
                                                                                 @Nullable Set<String> missingMaterials,
                                                                                 @Nullable Set<ResourceLocation> materials,
        ...
        if(!(model instanceof BlockModel)){
            ...
            if(materials == null || missingMaterials == null)
                model.getTextures(trackingResolver, new LinkedHashSet<>());
            else
                materials.addAll(model.getTextures(trackingResolver, missingMaterials));
            ...
            if(geometry != null){
                ...
                for(com.supermartijn642.fusion.api.util.Either<String,ModelMaterial> entry : geometry.getRequiredMaterials()){
                    if(entry.isLeft())
                        ...
                        materials.add(entry.right().texture());
                }
            }
        ...
        if(requiredMaterialKeys != null && materials != null && missingMaterials != null){
            ...
            ModelGeometry.MaterialResolver materialResolver = ModelGeometry.MaterialResolver.fromKeyLookup(
                ...
                key -> missingMaterials.add(key + " in " + ((BlockModel)modelStack.stream().findFirst().orElseThrow(AssertionError::new)).name),
    ...
    public void resolve(Function<ResourceLocation,IUnbakedModel> resolver){
        ...
        this.isGui3d = this.model.getIsGui3d() == null || this.model.getIsGui3d();
    ...
    @Override
    public IBakedModel bake(ModelBakery modelBakery, BlockModel ignore, Function<ResourceLocation,TextureAtlasSprite> textureGetter, ISprite modelState, VertexFormat vertexFormat){
        // Collect warnings
