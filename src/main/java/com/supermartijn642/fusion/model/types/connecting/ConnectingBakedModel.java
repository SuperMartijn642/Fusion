...
public class ConnectingBakedModel extends WrappedBakedModel {
    ...
    public void emitBlockQuads(BlockAndTintGetter blockView, BlockState state, BlockPos pos, Supplier<Random> randomSupplier, RenderContext context){
        ...
        if(quads == null){
            ...
            context.pushTransform(quad -> {
                quad.copyTo(meshEmitter);
                meshEmitter.material(quad.material());
                meshEmitter.emit();
                return true;
            });
            ...
            context.pushTransform(quad -> this.remapQuad(quad, spriteFinder.find(quad, 0), data));
            ...
            // Submit the quads
            context.meshConsumer().accept(quads);
        }
    }
    ...
    private static void adjustVertexDataUV(MutableQuadView quad, int newU, int newV, TextureAtlasSprite sprite){
        for(int i = 0; i < 4; i++){
            ...
            float u = quad.spriteU(i, 0) + width * newU;

            float height = sprite.getV1() - sprite.getV0();
            float v = quad.spriteV(i, 0) + height * newV;
            quad.sprite(i, 0, u, v);
        }
    }
