...
public abstract class BaseModelType<T extends BaseModelData, BUILDER extends BaseModelData.Builder<BUILDER,T>> implements ModelType<T> {
    ...
    public BlockStateModel bakeBlockStateModel(BlockStateModelBakingContext context, T data){
        ...
        context.walkModelTree(ModelInstance.of(this, data), (modelInstance, stack) -> {
            ...
            for(QuadAccess quad : quads.all()){
                if(quad.chunkLayer().translucent())
                    materialFlags |= 1;
                else if(quad.sprite().contents().isAnimated())
                    materialFlags |= 2;
            }
            // Create a new part
        ...
        int materialFlags = parts.stream().mapToInt(BaseBlockStateModel.Part::materialFlags).reduce(0, (a, b) -> a | b);
    ...
    public ItemModel bakeItemModel(ItemModelBakingContext context, T data){
        ...
        return new BaseItemModel(parts, context.getTintSources(), context.getTransformation());
    }
    ...
    public static void applyTextureProperties(MutableQuad quad){
        ...
        if(textureInstance != null && textureInstance.getCustomData() instanceof BaseTextureData data){
            if(data.isEmissive())
                quad.emissive(true);
            FusionClient.getChunkLayer(data.getRenderType()).ifPresent(quad::chunkLayer);
    ...
    protected void deserialize(JsonObject json, BUILDER builder){
        ...
        if(json.has("parent")){
            ...
            builder.parent(Identifier.parse(value));
        }
        // Materials
        ...
        if(json.has("gui_light")){
            ...
            try{
                ...
                String allowedValues = Arrays.stream(UnbakedModel.GuiLight.values()).map(o -> "'" + o.name().toLowerCase(Locale.ROOT) + "'").collect(Collectors.joining(","));
        ...
        if(json.has("display")){
            ...
            ItemTransform fixedFromBottom = this.deserializeItemTransform(displayJson, ItemDisplayContext.ON_SHELF);
            // Modded transforms
        ...
        for(String key : json.keySet()){
            if(json.get(key).isJsonPrimitive() && json.getAsJsonPrimitive(key).isString()){
                ...
                if(!value.isEmpty() && value.charAt(0) == '#')
                    ...
                    materials.put(key, Either.right(ModelMaterial.of(Identifier.parse(value), false)));
                else
    ...
    protected JsonObject serializeMaterials(Map<String,Either<String,ModelMaterial>> materials){
        ...
        for(Map.Entry<String,Either<String,ModelMaterial>> entry : materials.entrySet()){
            ...
            else if(!entry.getValue().right().forceTranslucent())
                materialsJson.addProperty(entry.getKey(), entry.getValue().right().texture().toString());
            else{
                JsonObject material = new JsonObject();
                material.addProperty("sprite", entry.getValue().right().texture().toString());
                material.addProperty("force_translucent", entry.getValue().right().forceTranslucent());
            }
        }
    ...
    protected CuboidRotation deserializeRotation(JsonObject json){
        if(!json.has("origin"))
            throw new JsonParseException("Element rotation must have array property 'origin'!");
        Vector3fc origin = this.deserializeVector3f(json.get("origin"), () -> "Element rotation property 'origin'").div(16);
        ...
        if(json.has("axis") || json.has("angle")){
            if(!json.has("axis") || !json.has("angle"))
                throw new JsonParseException("Element rotation must have both 'axis' and 'angle' properties!");
            ...
            rotationValue = new CuboidRotation.SingleAxisRotation(axis, json.get("angle").getAsFloat());
        }else if(json.has("x") || json.has("y") || json.has("z")){
            float x = 0, y = 0, z = 0;
            if(json.has("x")){
                if(!json.get("x").isJsonPrimitive() || !json.getAsJsonPrimitive("x").isNumber())
                    throw new JsonParseException("Element rotation property 'x' must be a number!");
                x = json.getAsJsonPrimitive("x").getAsFloat();
            }
            if(json.has("y")){
                if(!json.get("y").isJsonPrimitive() || !json.getAsJsonPrimitive("y").isNumber())
                    throw new JsonParseException("Element rotation property 'y' must be a number!");
                y = json.getAsJsonPrimitive("y").getAsFloat();
            }
            if(json.has("z")){
                if(!json.get("z").isJsonPrimitive() || !json.getAsJsonPrimitive("z").isNumber())
                    throw new JsonParseException("Element rotation property 'z' must be a number!");
                z = json.getAsJsonPrimitive("z").getAsFloat();
            }
            rotationValue = new CuboidRotation.EulerXYZRotation(x, y, z);
        }else
            throw new JsonParseException("Element rotation must have 'axis' and 'angle', or individual 'x', 'y', 'z' properties!");
    ...
    protected JsonObject serializeRotation(CuboidRotation rotation){
        ...
        }else if(rotation.value() instanceof CuboidRotation.EulerXYZRotation(float x, float y, float z)){
            if(x != 0)
                rotationJson.addProperty("x", x);
            if(y != 0)
                rotationJson.addProperty("y", y);
            if(z != 0)
                rotationJson.addProperty("z", z);
        }
        if(rotation.rescale())
    ...
    protected CuboidModelGeometry.Face.Builder deserializeFace(JsonObject json){
        ...
        if(json.has("rotation")){
            ...
            builder.rotation(Quadrant.values()[(rotation / 90) % 4]);
        }
    ...
    protected JsonObject serializeFace(CuboidModelGeometry.Face face){
        ...
        if(face.rotation() != null)
            faceJson.addProperty("rotation", face.rotation().shift * 90);
