...
public class ConnectingModelQuads {
    ...
    public ConnectingModelQuads(List<Entry>[] quads){
        ...
        for(int i = 0; i < 7; i++){
            if(quads[i] == null){
                this.quads[i] = List.of();
                continue;
            }
            this.quads[i] = List.copyOf(quads[i]);
            all.addAll(quads[i]);
        }
        this.all = List.copyOf(all);
    }

    public List<Entry> get(@Nullable Direction cullDirection){
    ...
    public static class Entry {
        ...
        private final ConnectingBlockStateModel.TextureOrientation orientation;
        private final ConnectingBlockStateModel.QuadPredicatesKey predicatesKey;

        public Entry(QuadAccess quad, ConnectionPredicate connectionPredicate){
            this.quad = quad;
            this.connectionPredicate = connectionPredicate;
            this.orientation = ConnectingBlockStateModel.findOrientation(quad);
            this.predicatesKey = new ConnectingBlockStateModel.QuadPredicatesKey(quad.facing(), this.orientation, connectionPredicate);
        }
        ...
        ConnectingBlockStateModel.TextureOrientation orientation(){
            return this.orientation;
        }

        ConnectingBlockStateModel.QuadPredicatesKey predicateKey(){
    ...
    public static class Builder {
        ...
        public Builder add(@Nullable Direction cullDirection, QuadAccess quad, ConnectionPredicate predicate){
