...
public class TextureCreationHandler {
    ...
    public static Result<SpriteContents> onLoadTexture(ResourceLocation identifier, NativeImage image, Optional<AnimationMetadataSection> animationMetadata, ResourceMetadata resourceMetadata){
        // Get the fusion metadata
        //noinspection unchecked,rawtypes
        RawTextureInstance<Object,Object> rawTexture = (RawTextureInstance)resourceMetadata.getSection(FusionTextureMetadataSection.TYPE).orElse(null);
        if(rawTexture == null)
            return null;
        ...
        for(DummyTextureSpriteContents.Child child : contents.children()){
            ...
            if(constructor == null){
                newSprite = new TextureAtlasSprite(
                    ...
                    new FusionSpriteContents(identifier, (SpriteImageSourceImpl)spriteBuilder.getImageSource(), contents.metadata()),
                    atlasWidth,
                    atlasHeight,
                    currentSprite.getX(),
                    currentSprite.getY()
