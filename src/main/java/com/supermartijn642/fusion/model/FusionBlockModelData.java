...
public class FusionBlockModelData extends BlockModel {
    ...
    public static Collection<RenderMaterial> gatherBlockModelMaterials(BlockModel model,
        ...
        ModelGeometry.MaterialKeyResolver.fromKeyLookup(
            ...
            material -> {
                materials.add(material.toRenderMaterial());
                return null;
            },
    ...
    private static Map<ResourceLocation,IUnbakedModel> resolveAndGatherMaterials(IUnbakedModel model,
                                                                                 ...
                                                                                 @Nullable Set<RenderMaterial> materials,
            ...
            if(geometry != null){
                ...
                for(com.supermartijn642.fusion.api.util.Either<String,ModelMaterial> entry : geometry.getRequiredMaterials()){
                    if(entry.isLeft())
                        ...
                        materials.add(entry.right().toRenderMaterial());
                }
            }
    ...
    private void setDependencies(Map<ResourceLocation,IUnbakedModel> dependencies){
        ImmutableMap.Builder<ResourceLocation,UntypedModelInstance> builder = ImmutableMap.builder();
        dependencies.forEach((key, model) -> builder.put(key, getModelInstance(model)));
        this.dependencies = builder.build();
    }

    @Override
    public IBakedModel bake(ModelBakery modelBakery, Function<RenderMaterial,TextureAtlasSprite> textureGetter, IModelTransform modelState, ResourceLocation topModelIdentifier){
        // Collect warnings
        ...
        if(bakedModel == null){
            ...
            TextureAtlasSprite particleSprite = ModelGeometry.MaterialKeyResolver.fromKeyLookup(
                key -> ModelStack.empty().push(this.model, this.identifier).findMaterialIncludingParents(key, context),
                context::getMaterial,
                s -> {},
                s -> {}
            ).get("particle", false);
    ...
    @Override
    public Either<RenderMaterial,String> findTextureEntry(String key){
        Either<RenderMaterial,String> material = getFromModelTree(
            this,
            ...
            m -> {
                ...
                return entry == null ? null : entry.flatMap(
                    Either::right,
                    mm -> Either.left(mm.toRenderMaterial())
                );
            },
            null
        );
        if(material == null)
            return Either.left(ModelMaterial.missingBlockAtlas().toRenderMaterial());
        return material;
    }
    ...
    private static Map<String,Either<RenderMaterial,String>> getMaterials(UntypedModelInstance model){
        Map<String,Either<RenderMaterial,String>> materials = new HashMap<>();
        model.getMaterials().forEach((key, value) -> {
            if(value.isLeft())
                materials.put(key, Either.right(value.left()));
            else
                materials.put(key, Either.left(value.right().toRenderMaterial()));
        });
    ...
    public static boolean containsFusionModelsOrTextures(IUnbakedModel model, SpriteMap atlasSet){
        ...
        if(atlasSet != null){
            for(Map.Entry<String,Either<RenderMaterial,String>> entry : ((BlockModel)model).textureMap.entrySet()){
                if(entry.getValue().left().isPresent()){
                    RenderMaterial material = entry.getValue().left().orElseThrow(AssertionError::new);
                    AtlasTexture atlas = atlasSet.getAtlas(material.atlasLocation());
                    if(atlas == null)
                        continue;
