...
public class OrientedMutableQuad extends MutableQuadImpl {
    ...
    public MutableQuad copyBakedQuad(BakedQuad quad){
        ...
        for(int i = 0; i < 4; i++){
            this.position(i,
                BakedQuadHelper.getPosX(quad.vertices(), i),
                BakedQuadHelper.getPosY(quad.vertices(), i),
                BakedQuadHelper.getPosZ(quad.vertices(), i)
            );
            this.uv(i,
                BakedQuadHelper.getU(quad.vertices(), i),
                BakedQuadHelper.getV(quad.vertices(), i)
            );
        }
        return this;
    }
