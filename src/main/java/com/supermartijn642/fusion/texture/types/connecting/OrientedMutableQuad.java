...
public class OrientedMutableQuad extends MutableQuadImpl {
    ...
    public MutableQuad copyBakedQuad(BakedQuad quad){
        ...
        for(int i = 0; i < 4; i++){
            this.position(i,
                BakedQuadHelper.getPosX(format, quad.getVertices(), i),
                BakedQuadHelper.getPosY(format, quad.getVertices(), i),
                BakedQuadHelper.getPosZ(format, quad.getVertices(), i)
            );
            this.uv(i,
                BakedQuadHelper.getU(format, quad.getVertices(), i),
                BakedQuadHelper.getV(format, quad.getVertices(), i)
            );
        }
        return this;
    }
