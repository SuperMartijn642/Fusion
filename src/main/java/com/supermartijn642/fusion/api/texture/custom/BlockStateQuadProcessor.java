...
/**
 * Processor for quads in a block state model.
 * <p>
 * Created 12/05/2026 by SuperMartijn642
 * @see TextureType#initializeBlockStateModelQuad(MutableQuad, SpriteInstance, Object, PropertyStore)
 */
public interface BlockStateQuadProcessor<S> {
     ...
     */
    S extractState(@Nullable BlockAndTintGetter level, @Nullable BlockPos pos, @Nullable BlockState state, Supplier<RandomSource> randomSupplier, PropertyStore properties);
     ...
     * @param state      state obtained from {@link #extractState(BlockAndTintGetter, BlockPos, BlockState, Supplier, PropertyStore)}
     * @param properties property store containing model properties as well as shared properties. May be used to store data shared between multiple quads. Any stored properties are released after rendering.
     * @see BlockStateModelExtension#createGeometryKey(BlockAndTintGetter, BlockPos, BlockState, RandomSource)
     */
    @Nullable
