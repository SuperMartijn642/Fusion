...
public abstract class BaseModelType<T extends BaseModelData, BUILDER extends BaseModelData.Builder<BUILDER,T>> extends SimpleModelType<T> {
    ...
    protected CuboidModelGeometry.Element.Builder deserializeElement(JsonObject json){
        ...
        for(Map.Entry<String,JsonElement> entry : facesJson.entrySet()){
            EnumFacing side = EnumFacing.byName(entry.getKey().toLowerCase(Locale.ROOT));
            if(side == null){
                String allowedValues = Arrays.stream(EnumFacing.values()).map(o -> "'" + o.getName() + "'").collect(Collectors.joining(","));
    ...
    protected JsonObject serializeElement(CuboidModelGeometry.Element element){
        ...
        for(EnumFacing side : EnumFacing.values()){
            ...
            if(face != null)
                facesJson.add(side.getName(), this.serializeFace(face));
        }
    ...
    protected BlockPartRotation deserializeRotation(JsonObject json){
        ...
        EnumFacing.Axis axis = EnumFacing.Axis.byName(json.get("axis").getAsString().toLowerCase(Locale.ROOT));
        if(axis == null){
            String allowedValues = Arrays.stream(EnumFacing.Axis.values()).map(o -> "'" + o.getName() + "'").collect(Collectors.joining(","));
    ...
    protected CuboidModelGeometry.Face.Builder deserializeFace(JsonObject json){
        ...
        if(json.has("cullface")){
            ...
            EnumFacing cullface = EnumFacing.byName(json.get("cullface").getAsString().toLowerCase(Locale.ROOT));
            if(cullface == null){
                String allowedValues = Arrays.stream(EnumFacing.values()).map(o -> "'" + o.getName() + "'").collect(Collectors.joining(","));
    ...
    protected JsonObject serializeFace(CuboidModelGeometry.Face face){
        ...
        if(face.cullDirection() != null)
            faceJson.addProperty("cullface", face.cullDirection().getName());
    ...
    protected ItemCameraTransforms deserializeItemTransforms(JsonObject json){
        ...
        if(thirdPersonLeftHand == ItemTransformVec3f.DEFAULT)
        ...
        if(firstPersonLeftHand == ItemTransformVec3f.DEFAULT)
    ...
    protected ItemTransformVec3f deserializeItemTransform(JsonObject json, String name, ItemCameraTransforms.TransformType type){
        ...
        Vector3f rotation = object.has("rotation") ?
            ...
            new Vector3f(ItemTransformVec3f.Deserializer.ROTATION_DEFAULT);
        Vector3f translation = object.has("translation") ?
            ...
            new Vector3f(ItemTransformVec3f.Deserializer.TRANSLATION_DEFAULT);
        translation.scale(1 / 16f);
        translation.set(MathHelper.clamp(translation.x, -5, 5), MathHelper.clamp(translation.y, -5, 5), MathHelper.clamp(translation.z, -5, 5));
        Vector3f scale = object.has("scale") ?
            this.deserializeVector3f(object.get("scale"), () -> "Display '" + name + "' property 'scale'") :
            new Vector3f(ItemTransformVec3f.Deserializer.SCALE_DEFAULT);
        scale.set(MathHelper.clamp(scale.x, -4, 4), MathHelper.clamp(scale.y, -4, 4), MathHelper.clamp(scale.z, -4, 4));
    ...
    protected JsonObject serializeItemOverride(ItemOverride override){
        JsonObject json = new JsonObject();
        json.addProperty("model", override.getLocation().toString());
        JsonObject predicatesJson = new JsonObject();
        override.mapResourceValues.forEach((k, v) -> predicatesJson.addProperty(k.toString(), v));
