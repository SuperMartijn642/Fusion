 ...
 */
public abstract class FusionItemModelModifierProvider implements IDataProvider {
    ...
    public final void run(DirectoryCache cache) throws IOException{
        ...
        for(Map.Entry<ResourceLocation,ModifierBuilder> entry : this.modifiers.entrySet()){
            ...
            Path path = Paths.get("assets", location.getNamespace(), "fusion/model_modifiers/items", location.getPath() + extension);
    ...
    private JsonObject toJson(ModifierBuilder modifier){
        ...
        for(Pair<ResourceLocation,ItemPredicate> pair : modifier.conditionalModels){
            ...
            List<ItemPredicate> predicates = pair.right() instanceof AndItemPredicate ? ((AndItemPredicate)pair.right()).getPredicates() : Collections.singletonList(pair.right());
            predicates.stream()
