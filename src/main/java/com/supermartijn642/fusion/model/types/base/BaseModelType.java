...
public class BaseModelType implements ModelType<BaseModelData> {
    ...
    public BakedModel bake(ModelBakingContext context, BaseModelData data){
        ...
        ItemTransform transformFirstPersonLeftHand = ((BaseModelDataImpl)data).findProperty(context, model -> model.transforms.hasTransform(ItemTransforms.TransformType.FIRST_PERSON_LEFT_HAND) ? model.transforms.getTransform(ItemTransforms.TransformType.FIRST_PERSON_LEFT_HAND) : null, ItemTransform.NO_TRANSFORM);
        ItemTransform transformFirstPersonRightHand = ((BaseModelDataImpl)data).findProperty(context, model -> model.transforms.hasTransform(ItemTransforms.TransformType.FIRST_PERSON_RIGHT_HAND) ? model.transforms.getTransform(ItemTransforms.TransformType.FIRST_PERSON_RIGHT_HAND) : null, ItemTransform.NO_TRANSFORM);
        ItemTransform transformHead = ((BaseModelDataImpl)data).findProperty(context, model -> model.transforms.hasTransform(ItemTransforms.TransformType.HEAD) ? model.transforms.getTransform(ItemTransforms.TransformType.HEAD) : null, ItemTransform.NO_TRANSFORM);
        ItemTransform transformGui = ((BaseModelDataImpl)data).findProperty(context, model -> model.transforms.hasTransform(ItemTransforms.TransformType.GUI) ? model.transforms.getTransform(ItemTransforms.TransformType.GUI) : null, ItemTransform.NO_TRANSFORM);
        ItemTransform transformGround = ((BaseModelDataImpl)data).findProperty(context, model -> model.transforms.hasTransform(ItemTransforms.TransformType.GROUND) ? model.transforms.getTransform(ItemTransforms.TransformType.GROUND) : null, ItemTransform.NO_TRANSFORM);
