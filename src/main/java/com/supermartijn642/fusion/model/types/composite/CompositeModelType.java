...
public class CompositeModelType implements ModelType<CompositeModelData> {
    ...
    @Override
    public BlockModel.@Nullable GuiLight getGuiLight(CompositeModelData data){
        return data.getGuiLight();
    }

    @Override
    public @Nullable ItemTransform getItemTransform(ItemTransforms.TransformType type, CompositeModelData data){
    ...
    @Override
    public @Nullable BakedModel bakeModel(ModelBakingContext context, ModelStack modelStack, CompositeModelData data){
        // Bake model entries
        ...
        for(List<CompositeModelData.ModelEntry> series : data.getModels()){
            ...
            for(CompositeModelData.ModelEntry entry : series){
                // Bake model
                ResourceLocation identifier = entry.getModel().leftOrNull();
                UntypedModelInstance model = entry.getModel().flatMap(
                    context::getModelOrMissing,
                    m -> m
                );
                BakedModel baked = model.bakeModel(context, modelStack.push(model, identifier));
                // Simply conditions
            ...
            lists.add(new CompositeBakedModel.ConditionalList(List.copyOf(list)));
        }

        // Create model
        if(defaultModel == null)
            defaultModel = context.getMissingBakedModel();
        return new CompositeBakedModel(defaultModel, List.copyOf(lists));
    }
    ...
    private static ModelTransform deserializeModelTransform(JsonArray array){
        ...
        for(JsonElement entry : array){
            ...
            matrix.multiply(deserializeModelTransformEntry(entry.getAsJsonObject()));
        }
    ...
    private static Matrix4f deserializeModelTransformEntry(JsonObject json){
        ...
        switch(type){
            case "translate" -> {
                ...
                return Matrix4f.createTranslateMatrix(translation[0] / 16, translation[1] / 16, translation[2] / 16);
            }
            ...
            case "scale" -> {
                ...
                return Matrix4f.createScaleMatrix(scaling[0], scaling[1], scaling[2]);
            }
            ...
            case "rotate" -> {
                ...
                if(json.has("rotation")){
                    ...
                    switch(axis){
                        case "x" -> matrix = MatrixUtil.createXRotationMatrix(angle);
                        case "y" -> matrix = MatrixUtil.createYRotationMatrix(angle);
                        case "z" -> matrix = MatrixUtil.createZRotationMatrix(angle);
                ...
                if(json.has("origin")){
                    ...
                    m.multiply(matrix);
                    m.multiply(Matrix4f.createTranslateMatrix(origin[0], origin[1], origin[2]));
                    return m;
                }
                return matrix;
            }
