    ...
    public IBakedModel bake(ModelBakingContext context, ConnectingModelData data){
        ...
        predicates.putAll(
            data.getAllConnectionPredicates().entrySet().stream()
                .map(entry -> Pair.of(entry.getKey().equals("default") ? DEFAULT_CONNECTION_KEY : new ResourceLocation(data.getVanillaModel().getTexture(entry.getKey())), entry.getValue()))
        ...
        return new ConnectingBakedModel(model, context.getTransformation().getState().apply(Optional.empty()).orElse(TRSRTransformation.identity()), predicates.build());
    }
