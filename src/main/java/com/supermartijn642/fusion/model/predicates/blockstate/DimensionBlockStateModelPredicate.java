...
public class DimensionBlockStateModelPredicate implements BlockStateModelPredicate {
    ...
    public static BlockStateModelPredicate create(ResourceKey<Level>... dimensions){
        return new DimensionBlockStateModelPredicate(Arrays.stream(dimensions).map(ResourceKey::location).collect(Collectors.toSet()));
    }
    ...
    public static final Serializer<DimensionBlockStateModelPredicate> SERIALIZER = new Serializer<>() {
        ...
        public DimensionBlockStateModelPredicate deserialize(JsonObject json) throws JsonParseException{
            ...
            if(json.has("dimension")){
                ...
                dimensions = Set.of(ResourceLocation.parse(json.get("dimension").getAsString()));
            }else{
    ...
    private DimensionBlockStateModelPredicate(Collection<ResourceLocation> dimensions){
        ...
        this.containsOverworld = this.dimensions.contains(Level.OVERWORLD.location());
    }

    @Override
    public boolean test(@Nullable BlockAndTintGetter level, @Nullable BlockPos pos, @Nullable BlockState state){
        if(!(level instanceof Dimensional dimensional))
            return this.containsOverworld;
        return this.dimensions.contains(dimensional.fusionGetDimension());
    }
