...
public interface ModelStack extends Iterable<UntypedModelInstance> {
     ...
     */
    ModelStack push(UntypedModelInstance instance, @Nullable Identifier identifier);
     ...
     */
    @Nullable
    UnbakedModel.GuiLight findGuiLightIncludingParents(ModelResolver modelResolver);
    ...
    /**
     * Finds the first model in the stack and the model tree of the last model that returns a non-null value for {@link ModelInstance#getItemTransform(ItemDisplayContext)} and returns its value.
     * If no such model is present, the result is {@code null}.
     */
    @Nullable
    ItemTransform findItemTransformIncludingParents(ItemDisplayContext type, ModelResolver modelResolver);
