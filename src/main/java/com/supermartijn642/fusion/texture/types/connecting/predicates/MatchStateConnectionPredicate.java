...
public class MatchStateConnectionPredicate implements ConnectionPredicate {
    ...
    public static ConnectionPredicate create(BlockState state){
        //noinspection unchecked
        return new MatchStateConnectionPredicate(state.getBlock(), state.getProperties().stream().map(p -> Pair.of(p, ImmutableSet.of(state.getValue(p)))).toArray(Pair[]::new));
    }
    ...
    public static <T extends Comparable<T>> Set<BlockState> computeStates(Block block, Pair<IProperty<?>,Set<?>>[] properties){
        // Compute the number of states matching this predicate
        Set<IProperty<?>> unrestrictedProperties = new HashSet<>(block.getStateDefinition().getProperties());
        int validStates = 1;
        ...
        Set<BlockState> resolvedStates = states.collect(ImmutableSet.toImmutableSet());
        // Sanity check
