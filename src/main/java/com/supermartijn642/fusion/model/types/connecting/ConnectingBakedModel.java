...
public class ConnectingBakedModel implements IBakedModel, CustomRenderTypeBakedModel {
    ...
    public static final ModelProperty<IEnviromentBlockReader> LEVEL_PROPERTY = new ModelProperty<>();
    ...
    public @NotNull List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction cullDirection, @NotNull Random random, @NotNull IModelData modelData){
        IEnviromentBlockReader level = modelData.getData(LEVEL_PROPERTY);
    ...
    private static boolean shouldConnect(ConnectionPredicate predicate, SurroundingBlockCache blocks, Direction face, ConnectionDirection direction, int neighborX, int neighborY, int neighborZ, BlockPos.MutableBlockPos mutablePos){
        IEnviromentBlockReader level = blocks.getLevel();
