...
public class RandomTextureType implements TextureType<RandomTextureData,StitchedRandomTextureData> {
    ...
    public @Nullable QuadProcessor<?> initializeModelQuad(MutableQuad quad, SpriteInstance sprite, StitchedRandomTextureData data, PropertyStore properties){
        ...
        int defaultIndex = calculateTileIndex(subTextures.size(), side, null, RandomSource.createNewThreadLocalInstance(), data);
        ...
        return new QuadProcessor<Pair<Integer,Object>>() {
            ...
            @Override
            public Pair<Integer,Object> extractState(ItemStack stack, Supplier<RandomSource> randomSupplier, PropertyStore properties){
                QuadProcessor<Object> subProcessor = subProcessors[defaultIndex];
    ...
    private static int calculateTileIndex(int tileCount, Direction side, @Nullable BlockPos pos, RandomSource random, RandomTextureData data){
        if(pos == null)
            pos = BlockPos.ZERO;
        // Calculate seed
        long seed = 1;
