...
public class ConnectingBakedModel implements IBakedModel, CustomRenderTypeBakedModel {
    ...
    public static final ModelProperty<IBlockDisplayReader> LEVEL_PROPERTY = new ModelProperty<>();
    ...
    public ConnectingBakedModel(List<Part> parts, TextureAtlasSprite particleSprite, boolean ambientOcclusion, BlockModel.GuiLight guiLight, boolean isGui3d, ItemCameraTransforms transforms){
        this.parts = parts;
    ...
    public @NotNull IModelData getModelData(@NotNull IBlockDisplayReader level, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull IModelData modelData){
        return new ModelDataMap.Builder()
            .withInitial(LEVEL_PROPERTY, level)
            .withInitial(POSITION_PROPERTY, pos)
            .build();
    }
    ...
    public @NotNull List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction cullDirection, @NotNull Random random, @NotNull IModelData modelData){
        IBlockDisplayReader level = modelData.getData(LEVEL_PROPERTY);
        ...
        SurroundingBlockCache blockCache = level == null || pos == null ? null : new SurroundingBlockCache(level, pos, state);
        // Collect quads
        ...
        for(Part part : this.parts){
            for(ConnectingModelQuads.Entry entry : part.quads.get(cullDirection)){
                ...
                if(textureType == DefaultTextureTypes.CONNECTING){
                    // If no blocks, then just emit the quad as is
                    if(blockCache == null){
                        bakedQuads.add(quad.toBakedQuad());
                        continue;
                    }
    ...
    public List<Pair<IBakedModel,RenderType>> getLayerModels(ItemStack stack, boolean fabulous){
        ...
        if(stack.getItem() instanceof BlockItem && RenderTypeLookup.getChunkRenderType(((BlockItem)stack.getItem()).getBlock().defaultBlockState()) != RenderType.translucent())
    ...
    static class QuadPredicatesKey {
        ...
        public final boolean equals(Object o){
            ...
            if(!(o instanceof QuadPredicatesKey)) return false;

            QuadPredicatesKey that = (QuadPredicatesKey)o;
    ...
    public static class Part {
        private final ConnectingModelQuads quads;

        public Part(ConnectingModelQuads quads){
            this.quads = quads;
        }
    }
}
