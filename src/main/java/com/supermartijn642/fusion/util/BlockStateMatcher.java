...
public class BlockStateMatcher {
    ...
    public static BlockStateMatcher create(Block block, Pair<IProperty<?>,?>... properties){
        ...
        for(Map.Entry<IProperty<?>,List<Object>> entry : propertyMap.entrySet())
            flattenedProperties[index++] = Pair.of(entry.getKey(), ImmutableSet.copyOf(entry.getValue()));
    ...
    private static <T extends Comparable<T>> Set<BlockState> computeStates(Block block, Pair<IProperty<?>,Set<?>>[] properties){
        ...
        Set<BlockState> resolvedStates = states.collect(Collectors.toSet());
        // Sanity check
