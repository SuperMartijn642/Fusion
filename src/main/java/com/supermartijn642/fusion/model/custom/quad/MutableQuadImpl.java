...
public class MutableQuadImpl implements MutableQuad {
    ...
    public MutableQuad copyFrom(QuadAccess quad){
        ...
        for(int i = 0; i < 4; i++){
            ...
            this.uvs[i].set(quad.u(i), quad.v(i));
        }
    ...
    public MutableQuad copyBakedQuad(BakedQuad quad){
        ...
        for(int i = 0; i < 4; i++){
            BakedQuadHelper.getPosition(quad.vertices(), i, this.positions[i]);
            BakedQuadHelper.getUV(quad.vertices(), i, this.uvs[i]);
        }
    ...
    public MutableQuad copyFrapiQuad(QuadView quad){
        ...
        for(int i = 0; i < 4; i++){
            ...
            quad.copyUv(i, this.uvs[i]);
        }
        this.facing = null;
        this.sprite = Minecraft.getInstance().getModelManager().getAtlas(TextureAtlas.LOCATION_BLOCKS).spriteFinder().find(quad);
        this.tintIndex = quad.tintIndex();
        this.shade = !quad.material().disableDiffuse();
        ...
        // Our properties
        this.ambientOcclusion = quad.material().ambientOcclusion() != TriState.FALSE;
        this.emissive = quad.material().emissive();
    ...
    public void toFrapiQuad(MutableQuadView quad){
        ...
        for(int i = 0; i < 4; i++){
            ...
            quad.uv(i, this.uvs[i]);
        }
        quad.color(0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF);

        int lightmap = LightTexture.pack(this.lightEmission, this.lightEmission);
        ...
        quad.material(
            Renderer.get().materialFinder()
                .blendMode(BlendMode.fromRenderLayer(this.chunkRenderType))
                .disableDiffuse(!this.shade)
                .ambientOcclusion(this.ambientOcclusion ? TriState.TRUE : TriState.FALSE)
