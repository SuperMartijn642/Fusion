...
public class FusionSpriteContents extends SpriteContents {
    ...
    public FusionSpriteContents(ResourceLocation identifier, SpriteImageSourceImpl imageSource, List<MetadataSectionType.WithValue<?>> metadataSections){
        super(identifier, new FrameSize(imageSource.getFrameWidth(), imageSource.getFrameHeight()), imageSource.getImage(), Optional.empty(), metadataSections);
    ...
    protected @Nullable AnimatedTexture createAnimatedTexture(FrameSize frameSize, int imageWidth, int imageHeight, AnimationMetadataSection animationMetadataSection){
        return new AnimatedTexture(this.frameInfos, 0, this.imageSource.shouldInterpolateFrames()) {
            @Override
            protected void uploadFrame(int destinationX, int destinationY, int uniqueFrame, GpuTexture destination){
            ...
            public SpriteTicker createTicker(){
                ...
                InterpolationData interpolationData = new InterpolationData() {
                    ...
                    protected int getPixel(AnimatedTexture animatedTexture, int frameIndex, int mipLevel, int x, int y){
                        ...
                        return FusionSpriteContents.this.byMipLevel[mipLevel].getPixel(x + (frame.u() >> mipLevel), y + (frame.v() >> mipLevel));
                    }
                };
