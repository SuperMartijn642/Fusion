...
public class BlockModelModifierBakedModel implements BlockStateModel {
    ...
    public void collectParts(RandomSource random, List<BlockStateModelPart> parts, ModelData modelData){
        ...
        if(renderData.defaultModel != -1){
            ...
            if(!isBreakingOverlay || override.showBreakingOverlay)
                override.model.collectParts(random, parts, renderData.defaultModelData);
        }else
            this.original.collectParts(random, parts, renderData.defaultModelData);
        ...
        for(int i = 0; i < this.appendModels.size(); i++){
            ...
            if(!isBreakingOverlay || conditional.showBreakingOverlay){
                random.setSeed(seed);
                conditional.model.collectParts(random, parts, renderData.appendModelsData[i]);
            }
        }
    }
    ...
    public Material.Baked particleMaterial(@NotNull ModelData data){
        ...
        if(renderData.defaultModel != -1){
            ConditionalModel override = this.defaultModelOverrides.get(renderData.defaultModel);
            return override.model.particleMaterial(renderData.defaultModelData);
        }
        return this.original.particleMaterial(renderData.defaultModelData);
    }
    ...
    record ConditionalModel(BlockStateModel model, @Nullable BlockStateModelPredicate conditions, boolean showBreakingOverlay) {
    }
