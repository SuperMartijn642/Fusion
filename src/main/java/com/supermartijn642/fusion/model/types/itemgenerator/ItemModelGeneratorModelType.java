...
public class ItemModelGeneratorModelType implements ModelType<Void> {
        ...
        return (transformation, materialResolver) -> {
            ...
            for(int layerIndex = 0; layerIndex < ItemModelGenerator.LAYERS.size(); layerIndex++){
                ...
                if(ModelMaterial.isMissingSprite(sprite))
                    break;
                ClientHooks.fixItemModelSeams(ITEM_MODEL_GENERATOR.processFrames(layerIndex, layerName, sprite.contents()), sprite)
    ...
    @Override
    public <X, C> Optional<X> getProperty(ModelProperty<X,C> property, C context, Void data){
