...
public class LevelExtractorMixin {
    ...
    private BlockBreakingRenderState extractBlockDestroyAnimation(BlockBreakingRenderState breakingState, @Share("parts") LocalRef<List<BlockStateModelPart>> dummyPartsList){
        ...
        ModelData entityData = this.level.getModelDataManager().getAtOrEmpty(pos);
        ...
        try{
            ...
            this.modelsByRandomOffset.foreach(
                entry -> {
                    ...
                    ModelData modelData = entry.getModelData(this.level, pos, blockState, entityData);
                    entry.collectParts(this.randomSource, finalPartsList, modelData);
