...
public class RandomTextureType implements TextureType<RandomTextureData,RandomTextureData> {
    ...
    public @Nullable QuadProcessor<?> initializeModelQuad(MutableQuad quad, SpriteInstance sprite, RandomTextureData data, PropertyStore properties){
        ...
        return new QuadProcessor<Integer>() {
            ...
            @Override
            public Integer extractState(@Nullable IBlockAccess level, @Nullable BlockPos pos, @Nullable IBlockState state, Supplier<Random> randomSupplier, PropertyStore properties){
    ...
    private static int calculateTileIndex(SpriteInstance sprite, EnumFacing side, @Nullable BlockPos pos, Random random, RandomTextureData data){
        if(pos == null)
            pos = BlockPos.ORIGIN;
        // Calculate seed
        long seed = 1;
