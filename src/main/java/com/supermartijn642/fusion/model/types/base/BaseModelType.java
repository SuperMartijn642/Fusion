...
public class BaseModelType implements ModelType<BaseModelData> {
    ...
    public IBakedModel bake(ModelBakingContext context, BaseModelData data){
        ...
        boolean ambientOcclusion = ((BaseModelDataImpl)data).findProperty(context, model -> model.ambientOcclusion, true);
        boolean gui3d = ((BaseModelDataImpl)data).model.isGui3d();
        TextureAtlasSprite particleSprite = context.getTexture(((BaseModelDataImpl)data).findParticleSprite(context));
        ItemTransformVec3f transformThirdPersonLeftHand = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.THIRD_PERSON_LEFT_HAND) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.THIRD_PERSON_LEFT_HAND) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformThirdPersonRightHand = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformFirstPersonLeftHand = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformFirstPersonRightHand = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformHead = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.HEAD) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.HEAD) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformGui = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.GUI) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.GUI) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformGround = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.GROUND) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.GROUND) : null, ItemTransformVec3f.DEFAULT);
        ItemTransformVec3f transformFixed = ((BaseModelDataImpl)data).findProperty(context, model -> model.cameraTransforms.hasCustomTransform(ItemCameraTransforms.TransformType.FIXED) ? model.cameraTransforms.getTransform(ItemCameraTransforms.TransformType.FIXED) : null, ItemTransformVec3f.DEFAULT);
    ...
    public BaseModelData deserialize(JsonObject json) throws JsonParseException{
        ...
        for(int i = 0; i < model.elements.size(); i++){
            ...
            elements.add(new BaseModelElement(vanillaElement.positionFrom, vanillaElement.positionTo, vanillaElement.mapFaces, vanillaElement.partRotation, vanillaElement.shade, lightEmission));
        }
