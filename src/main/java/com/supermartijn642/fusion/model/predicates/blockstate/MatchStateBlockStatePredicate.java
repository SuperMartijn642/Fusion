...
public class MatchStateBlockStatePredicate implements BlockStateModelPredicate {
    ...
    public static BlockStateModelPredicate create(Collection<Block> blocks, List<BlockPos> offsets, Pair<IProperty<?>,?>... properties){
    ...
    public static BlockStateModelPredicate create(BlockState state, List<BlockPos> offsets){
        ...
        Pair<IProperty<?>,?>[] properties = state.getProperties().stream().map(p -> Pair.of(p, state.getValue(p))).toArray(Pair[]::new);
        return create(ImmutableList.of(state.getBlock()), offsets, properties);
    }
    ...
    public static final Serializer<MatchStateBlockStatePredicate> SERIALIZER = new Serializer<MatchStateBlockStatePredicate>() {
        ...
        public MatchStateBlockStatePredicate deserialize(JsonObject json) throws JsonParseException{
            ...
            if(json.has("offset")){
                ...
                offsets = ImmutableSet.of(new BlockPos(x, y, z));
                ...
                offsets = ImmutableSet.of(BlockPos.ZERO);
            return new MatchStateBlockStatePredicate(matchers, offsets);
        }
    ...
    private final BlockPos.Mutable dummyBlockPos = new BlockPos.Mutable();
    ...
    public boolean test(@Nullable ILightReader level, @Nullable BlockPos pos, @Nullable BlockState state){
        ...
        for(BlockPos offset : this.offsets){
            this.dummyBlockPos.set(pos).move(offset.getX(), offset.getY(), offset.getZ());
