...
public class ModelWalkerImpl {
    ...
    public static <T> Optional<T> walkModelTree(Function<Identifier,ModelInstance<?>> modelResolver, ModelInstance<?> modelInstance, ModelWalker<T> walker){
    ...
    public static <T> Optional<T> walkModelTree(Function<Identifier,ModelInstance<?>> modelResolver, Identifier model, ModelWalker<T> walker){
        ModelStackImpl stack = new ModelStackImpl();
    ...
    private static <T> ModelWalker.Result<T> walk(ModelStackImpl stack, Function<Identifier,ModelInstance<?>> modelResolver, Either<Identifier,ModelInstance<?>> entry, ModelWalker<T> walker){
        ...
        for(Either<Identifier,ModelInstance<?>> parent : modelInstance.getParents()){
    ...
    private static class ModelStackImpl implements ModelWalker.ModelStack {
        ...
        private final List<Pair<Identifier,ModelInstance<?>>> models = new ArrayList<>(); // Model trees aren't expected to get very tall, hence an array list should be fine

        void push(Identifier identifier, ModelInstance<?> modelInstance){
        ...
        public @NotNull Iterator<ModelInstance<?>> iterator(){
            Iterator<Pair<Identifier,ModelInstance<?>>> iterator = this.models.iterator();
            return new Iterator<>() {
                @Override
        ...
        public @Nullable UnbakedModel.GuiLight findGuiLight(){
            for(ModelInstance<?> model : this){
                UnbakedModel.GuiLight value = model.getGuiLight();
                if(value != null)
                    return value;
            }
            return null;
