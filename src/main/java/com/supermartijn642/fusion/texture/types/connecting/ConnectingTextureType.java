...
public class ConnectingTextureType implements TextureType<ConnectingTextureData,StitchedConnectingTextureData> {
    ...
    public void createTexture(TextureOutput<StitchedConnectingTextureData> output, TextureCreationContext context, ConnectingTextureData data) throws UserErrorException{
        ...
        if(data.getLayout() == ConnectingTextureData.Layout.FULL && frameWidth == frameHeight){ // Legacy full layout was a square image, so change the framing to the new aspect ratio
            ...
            image = ImageHelper.createCrop(image, 0, 0, imageWidth, imageHeight);
        ...
        if(animationMetadata != null){
            ...
            animationMetadata = new AnimationMetadataSection(
                animationMetadata.animationFrames,
        ...
        for(int y = 0; y < layout.getHeight(); y++){
            for(int x = 0; x < layout.getWidth(); x++){
                ...
                if((x != layout.defaultTileX() || y != layout.defaultTileY()) &&
                    ...
                    continue;
                isEmpty = false;
                BufferedImage subImage = ImageHelper.createCropFramed(image, x * tileWidth, y * tileHeight, tileWidth, tileHeight, frameWidth, frameHeight);
    ...
    public @Nullable QuadProcessor<?> initializeModelQuad(MutableQuad quad, SpriteInstance sprite, StitchedConnectingTextureData data, PropertyStore properties){
        ...
        QuadProcessor<Object>[] subProcessors = new QuadProcessor[usedTileCount];
        ...
        for(int i = usedTiles.nextSetBit(0); i != -1; i = usedTiles.nextSetBit(i + 1)){
            ...
            for(int j = 0; j < 4; j++){
                subQuad.uv(
                    j,
                    defaultSprite.getU0() + (quad.u(j) - sprite.getU0()) / (sprite.getU1() - sprite.getU0()) * (defaultSprite.getU1() - defaultSprite.getU0()),
                    defaultSprite.getV0() + (quad.v(j) - sprite.getV0()) / (sprite.getV1() - sprite.getV0()) * (defaultSprite.getV1() - defaultSprite.getV0())
                );
            }
            ...
            if(subProcessor != null){
                //noinspection unchecked
                subProcessors[storageIndex] = (QuadProcessor<Object>)subProcessor;
                processorCount++;
            }
        }
        ...
        IntConsumer lazyQuadInitializer = tileIndex -> {
            ...
            synchronized(tile){
                ...
                for(int j = 0; j < 4; j++){
                    subQuad.uv(
                        j,
                        defaultSprite.getU0() + (quadCopy.u(j) - sprite.getU0()) / (sprite.getU1() - sprite.getU0()) * (defaultSprite.getU1() - defaultSprite.getU0()),
                        defaultSprite.getV0() + (quadCopy.v(j) - sprite.getV0()) / (sprite.getV1() - sprite.getV0()) * (defaultSprite.getV1() - defaultSprite.getV0())
                    );
                }
                subQuad.sprite(defaultSprite.getSprite());
                // Initialize sub quad
                QuadProcessor<?> subProcessor = tile.initializeModelQuad(subQuad, defaultSprite, PropertyStore.empty());
                if(subProcessor != null)
        ...
        return new QuadProcessor<Pair<TextureConnections,Object[]>>() {
            ...
            public Pair<TextureConnections,Object[]> extractState(Supplier<Random> randomSupplier, PropertyStore properties){
                ...
                while((i = contextlessTiles.nextSetBit(i + 1)) != -1){
                    byte storageIndex = tileToStorageIndex[i];
                    QuadProcessor<Object> tileProcessor = subProcessors[storageIndex];
                    if(tileProcessor == null)
                        continue;
                    tileStates[storageIndex] = tileProcessor.extractState(randomSupplier, properties);
                }
                return Pair.of(contextlessConnections, tileStates);
            }

            @Override
            public Pair<TextureConnections,Object[]> extractState(@Nullable IBlockAccess level, @Nullable BlockPos pos, @Nullable IBlockState state, Supplier<Random> randomSupplier, PropertyStore properties){
            ...
            public Pair<TextureConnections,Object[]> extractState(ItemStack stack, Supplier<Random> randomSupplier, PropertyStore properties){
                ...
                while((i = contextlessTiles.nextSetBit(i + 1)) != -1){
                    ...
                    if(tileProcessor == null)
                        continue;
                    tileStates[storageIndex] = tileProcessor.extractState(stack, randomSupplier, properties);
                }
                return Pair.of(contextlessConnections, tileStates);
            }
            ...
            public Object createGeometryKey(Pair<TextureConnections,Object[]> state, PropertyStore properties){
                ...
                if(state.left() == contextlessConnections){
                    ...
                    while((i = contextlessTiles.nextSetBit(i + 1)) != -1){
                        ...
                        if(tileProcessor == null)
                            continue;
                        Object subKey = tileProcessor.createGeometryKey(tileStates[storageIndex], properties);
                        if(subKey == null)
                            return null;
                        subKeys.add(subKey);
                    }
    ...
    private static TextureConnections computeConnections(QuadPredicatesKey predicates, SurroundingBlockCache blocks){
        ...
        EnumFacing face = predicates.side;
    ...
    private static boolean shouldConnect(ConnectionPredicate predicate, SurroundingBlockCache blocks, EnumFacing face, ConnectionDirection direction, int neighborX, int neighborY, int neighborZ, BlockPos.MutableBlockPos mutablePos){
    ...
    private static final class QuadPredicatesKey {
        ...
        private QuadPredicatesKey(EnumFacing side, TextureOrientation orientation, ConnectionPredicate predicate){
            this.side = side;
